    ...
    (function processItem() {
        try {
            ...
            for (var j = 0; j < imagewall[position][1].length; j++) {
                ...
                var slideImg = new Element("img", { src: imagewall[position][1][j][0] });
                slideImg.setStyles({
                    width: 'auto',
                    height: 'auto',
                    maxWidth: '100%',
                ...
                var desc = new Element("span", { html: imagewall[position][1][j][1] });
                var div = new Element("div").setProperty("class", "wall-item-description");
                desc.inject(div);
                div.inject(slide);
                    ...
                    if (imagewall[position][1].length) {
                        (function slideAnimation(item) {
                            ...
                            if (item.getNext("li")) {
                                ...
                                (function() { item.fade("out"); if (!stop) nextSlide(item.getNext("li")); }).delay(delay);
                            } else if (item.getSiblings("li").length > 0) {
                                var nextSlide = arguments.callee;
                                (function() { item.fade("out"); if (!stop) nextSlide(item.getSiblings("li").pop()); }).delay(delay);
                            }
